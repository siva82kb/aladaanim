...
def update_text():
    ...
    ax2.text(0.1, 1.12,
             r'Use up/down arrow to change $\lambda$ value.',
             fontsize=12, verticalalignment='center',
             horizontalalignment='left', color='gray', style='italic')
    ax2.text(0.1, 1.04,
    ...
    ax2.text(xpos, ypos - j * delypos, f"Netwon's Method",
             fontsize=fsz, backgroundcolor='tab:red', color='white')
    ...
    ax2.text(xpos, ypos - j * delypos, 
             f"$x_{{{k}}}$ = " + f"{_xk:0.3f}", 
             fontsize=fsz, color="tab:red")
    j += 1
    ax2.text(xpos, ypos - j * delypos, f"$f\\left(x_{{{k}}}\\right)$ = " + f"{f(_xk):0.3f}", 
             fontsize=fsz, color="tab:red")
    ax2.text(xpos + 0.4, ypos - j * delypos, f"$f'\\left(x_{{{k}}}\\right)$ = " + f"{df(_xk):0.3f}", 
             fontsize=fsz, color="tab:red")
    j += 1
    ax2.text(xpos, ypos - j * delypos, f"$f''\\left(x_{{{k}}}\\right)$ = " + f"{d2f(_xk):0.3f}", 
             fontsize=fsz, color="tab:red")
    j += 1
    ax2.text(xpos, ypos - j * delypos, f"$f'/f'' =$ {df(_xk) / d2f(_xk):0.3f}", 
             fontsize=fsz, color="tab:red")
    
    j += 1.5
    ax2.text(xpos, ypos - j * delypos, f"Levenberg-Marquardt Method",
             fontsize=fsz, backgroundcolor='tab:green', color='white')
    # Function value and its derivatives
    j += 1.2
    _xk = Xk[1, -1]
    ax2.text(xpos, ypos - j * delypos, f"$x_{{{k}}}$ = " + f"{_xk:0.3f}", 
             fontsize=fsz, color="tab:green")
    ax2.text(xpos + 0.4, ypos - j * delypos, f"$\\lambda$ = " + f"{l:0.1f}", 
             fontsize=fsz, color="tab:green")
    j += 1
    ax2.text(xpos, ypos - j * delypos, f"$f\\left(x_{{{k}}}\\right)$ = " + f"{f(_xk):0.3f}", 
             fontsize=fsz, color="tab:green")
    ...
    ax2.text(xpos, ypos - j * delypos, f"$f''\\left(x_{{{k}}}\\right)$ = " + f"{d2f(_xk):0.3f}", 
             fontsize=fsz, color="tab:green")
    ax2.text(xpos + 0.4, ypos - j * delypos, f"$f''\\left(x_{{{k}}}\\right) + \\lambda$ = " + f"{d2f(_xk) + l:0.3f}", 
             fontsize=fsz, color="tab:green")
    j += 1
    ax2.text(xpos, ypos - j * delypos, "$f'/(f'' + \\lambda) = $" + f"{df(_xk) / (d2f(_xk) + l):0.3f}", 
             fontsize=fsz, color="tab:green")

    # Minimum point
    j += 1.3
    ax2.text(xpos, ypos - j * delypos, f"$x^*$ = 0.0", fontsize=fsz)


# Handling key press events
def on_press(event):
    global funclass, methodclass, Xk, ak, l
